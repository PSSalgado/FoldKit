...
def main() -> None:
    ...
    summary_log = setup_log_from_args(
        ...
        inputs=list(getattr(args, "paths", []) or []),
        pattern=getattr(args, "pattern", None),
    )
    if summary_log is not None:
        summary_log.task("PDB rechain (pdb_rechain.py)")
        summary_log.kv("dry_run", bool(getattr(args, "dry_run", False)))
        summary_log.kv("output", getattr(args, "output", None) or "(default)")
        summary_log.kv("suffix", getattr(args, "suffix", None))
        summary_log.kv("pattern", getattr(args, "pattern", None))
        if getattr(args, "merge_map", None):
            summary_log.kv("merge_map", args.merge_map)
        else:
            summary_log.kv("from", getattr(args, "chain_from", None))
            summary_log.kv("to", getattr(args, "chain_to", None))
    ...
    if not inputs:
        ...
        if summary_log is not None:
            summary_log.error("No input PDB files.")
        sys.exit(2)
    if summary_log is not None:
        summary_log.kv("n_inputs", len(inputs))
