...
""",
    ...
    summary_log = setup_log_from_args(args, script_path=__file__, inputs=[getattr(args, "directory", "")], pattern=None)
    directory = os.path.abspath(args.directory)
    if summary_log is not None:
        summary_log.task("Rename files (rename_files.py)")
        summary_log.kv("directory", directory)
        summary_log.kv("recursive", not bool(args.no_recursive))
    ...
    if args.strip_prefix is not None:
        if summary_log is not None:
            summary_log.kv("mode", "legacy_strip_date")
            summary_log.kv("prefix", args.strip_prefix)
    ...
    if summary_log is not None:
        summary_log.kv("mode", "regex")
        summary_log.kv("remove", args.remove or "")
        summary_log.kv("replace", args.replace or "")
