...
def _parse_rmsd_number(line: str) -> float | None:
    ...
    # Coot LSQ can report RMS as "rms devi: 1.234" (deviation), not "rmsd".
    # Some logs also contain "RMSD = 1.234" style lines.
...
def parse_lsq_rmsd_txt(path: str) -> dict[tuple[str, str], float]:
    ...
    with open(path, "r") as f:
        for line in f:
            line = line.strip()
            # Two common LSQ log variants:
            #   1) "Alignment: Aligning A to B" then "core rmsd achieved: X"
            #   2) "Aligning A to B" then a block including "rms devi: X"
            if line.startswith("Alignment:"):
                text = line.replace("Alignment:", "", 1).strip()
            else:
                text = line
            ...
            elif current_pair and ("core rmsd" in line.lower() or "rms devi" in line.lower() or "rmsd" in line.lower()):
