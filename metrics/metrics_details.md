...
- **`bbox`** — fixed expanded-lattice / AABB supercell bands: Matthews **8–45**, solvent **80–100%**, packing **2–15%**. Occlusion: LOImolA **2–20%**, contact **2–25%**, BSAmolA/kDa **5–80 Å²**. Calibrated on a FoldKit multi-copy expanded-lattice cohort (including SlpA) and labelled for general supercell use. Replaces the old crystal×\(f\) inflation.
...
These five span LOI **8.9–12.1%**, contact **11.6–15.2%**, BSA/kDa **43.1–56.9 Å²**, and Matthews **7.73–8.96** (packing 11.9–13.8%, solvent 84.0–86.2%), giving `slayer-compact` volume **7–10 / 80–90 / 10–15** and occlusion **40–60 / 5–15 / 10–20**. The wider `bbox` occlusion bands (LOI **2–20%**, contact **2–25%**, BSA/kDa **5–80 Å²**) come from a broader multi-copy expanded-lattice cohort in which these SlpA lattices sit mid-band; because a 5-point grid would reduce positive empirical minima near 2% to zero, those lower limits use a finer 1%-point grid, producing nonzero baselines of **2%**. Upper limits use the 5-point grid. This is intentional: by definition an assembled lattice subunit makes neighbour contacts and occludes some residues, so a raw value of zero has no useful physical interpretation as the lower scoring anchor. BSA/kDa retains a 5-unit grid at both edges. Note that BSA/kDa is not independent of LOI (BSA/kDa = LOI × isolated SASA/kDa) and additionally varies with subunit size and architecture at similar LOI.
...
**Tunnel summary table.** One row per tunnel cluster included in the table (all clusters on a full run; selected clusters with `--summary-only`). Columns (in order): run path, `protein_pdb`, cluster id, snapshot, `tunnel_length_A`, `lining_chain_ids`, then for each of top / bottom / bottleneck: opening width (`width_*_diameter_A` by default, or `width_*_radius_A` with **`--radius`**) and `residues_*`. Width at top and bottom is 2 × Caver profile radius at the first and last centreline points (×1 with `--radius`). Bottleneck width uses `bottleneck_r` from `bottlenecks.csv`, with fallback to the profile bottleneck radius or minimum profile radius. Residue labels use the form `Lys123A` (title-case three-letter code, residue number, chain id), sorted by chain (alphabetical) then residue number (ascending), comma-separated. Top and bottom residues are shell-mapped from the structure file(s) supplied for that tunnel (same construction as plot annotations). Bottleneck residues are taken from Caver and normalised to the same label format when parseable. The output path is set with **`--summary-csv`** or **`--summary-table-csv`** (default names: `tunnels_summary_table.csv`, `tunnel_<N>_summary_table.csv`, or `tunnels_summary_combined.csv` depending on mode).

**Full run vs summary-only.** A **full run** writes `tunnels_summary_table.csv` (or `tunnel_<N>_summary_table.csv` when only one cluster exists in the directory) with every tunnel cluster, and writes `tunnel_<N>_ec.png` and `tunnel_<N>_points.csv` separately for each cluster named on the command line (`--tunnel N`, `--tunnels N,M,P`, or `--all-tunnels`). **`--summary-only`** writes the summary table for the selected cluster(s) only and does not produce plots or per-point CSV.
...
**Optional outputs and plotting.** A per-point CSV (default or `--output-csv`) lists distance along the path, opening width (`diameter_A` by default, or `radius_A` with `--radius`), EC values, local-window series (where enabled), and Kyte–Doolittle hydropathy; the per-point `residues` column uses `chain:res:aa` tokens. `--residues-csv` can list the union of mapped residues. `--json-out` writes a compact summary. Profile plots use **diameter** on the horizontal axis by default; annotation callouts use the same residue label format as the summary table. `--color-by ec` (default) or `hydropathy` selects the colour scale (`--colour-by` is an alias); `--no-plot` skips figures. Plots are produced with Matplotlib; the output format follows `--plot-out`. Distance sampling can be refined with `--plot-upsample` (capped by `--plot-upsample-max`); SciPy enables smoother resampling when installed.

**Axis orientation.** The profile plot uses distance along the tunnel on the y-axis. `--invert-y` (default) or `--no-invert-y` sets the default orientation for all tunnels. Per-cluster overrides: `--tunnel-invert N` (force invert) and `--tunnel-no-invert N` (force upright), repeatable; the same cluster id cannot appear in both lists.
**Shared plot axes (multi-tunnel).** When more than one profile plot is produced in one command, y-axis limits use the global min/max distance across those tunnels (longest span); shorter tunnels are drawn at true relative height with blank padding unless `--per-tunnel-plot-ylim` is set. X-axis limits use the widest tunnel’s half-width (diameter or radius per `--radius`) plus **1 Å** on each side, shared across plots, unless `--per-tunnel-plot-xlim` is set. `--xlim XMIN XMAX` fixes x for every plot and overrides the automatic shared x span.
...
- Alignment sources (in order): (1) **DaliLite** (if `--dalilite-path` or `DALILITE_HOME` is set), (2) `--alignment` file, (3) biotite structural alignment (if installed), (4) sequence-order matching (same residue numbering). When DaliLite returns a Z on the summary hit line, that **Dali Z** is stored in the **`z_score`** column; if it is absent, **FoldKit** fills **`z_score`** with the **empirical** normalisation in Section 6.2 (`compute_z_score`). Console output from **`dalilite_pairs.py`** labels **empirical Z-score** when the latter is used. **All-vs-all** input files are **natural-sorted** before running pairs; the **pairwise** export and **Z-matrix** follow that order; the **ranking** table is ordered **by Z** (average, then maximum), not by name.
...
- **Z-score** (Dali’s summary **Z** on the hit line, stored in CSV when present; if missing, **Section 6.2 empirical Z** is used for `z_score`)
...
- **Z-score matrix** (`--output-matrix`): symmetric matrix; row/column label order is **natural-sorted**
