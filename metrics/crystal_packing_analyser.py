...
# Import analysis modules (top-level folders; run from repo root)
try:
...
try:
    from metrics.interface_analyser_base import InterfaceAnalyser, InterfaceAnalyserEC
except ImportError:
...
try:
    from metrics.contact_analyser import ContactAnalyser
except ImportError:
...
Examples (from repository root):
  ...
  python metrics/crystal_packing_analyser.py --input *.pdb --sets set_a set_b --output analysis_output --dry-run
    ...
    if summary_log is not None:
        summary_log.task("Crystal packing analysis pipeline (crystal_packing_analyser.py)")
        summary_log.kv("output", getattr(args, "output", None))
        summary_log.kv("compare", bool(getattr(args, "compare", False)))
        summary_log.kv("interface_context", getattr(args, "interface_context", None))
        summary_log.kv("interface_metrics", getattr(args, "interface_metrics", None))
    ...
    for label, patterns, filtered in set_list:
        ...
        all_results = []
        for i, pdb_file in enumerate(filtered, start=1):
            if not os.path.exists(pdb_file):
                print(f"Warning: File {pdb_file} not found, skipping...", file=sys.stderr)
                if summary_log is not None:
                    summary_log.error(f"Missing file: {pdb_file}")
                continue
            if summary_log is not None:
                summary_log.progress(i, len(filtered), os.path.basename(pdb_file))
